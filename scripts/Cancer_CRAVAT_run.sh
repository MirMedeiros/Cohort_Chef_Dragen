...
# CLEAN VCF:
bcftools annotate -x ^FORMAT/AD,^FORMAT/DP,^FORMAT/FT,^FORMAT/GQ,^FORMAT/GT,^FORMAT/MIN_DP,^FORMAT/PGT,^FORMAT/PID,^FORMAT/PL,^FORMAT/PS,^FORMAT/RGQ,^FORMAT/SB,^INFO/AC,^INFO/AF,^INF
O/AN,^INFO/ANN,^INFO/BaseQRankSum,^INFO/DP,^INFO/END,^INFO/ExcessHet,^INFO/FS,^INFO/InbreedingCoeff,^INFO/MLEAC,^INFO/MLEAF,^INFO/MQ,^INFO/MQRankSum,^INFO/QD,^INFO/RAW_MQandDP,^INFO/ReadPosRankSum,^INFO/SOR FullCohort_FullQC.vcf.gz  -O z -o cleaned_fullQC.vcf.gz

# run
oc run cleaned_fullQC.vcf.gz  -l hg${Genome_build} -a alphamissense bayesdel cadd clinvar clingen ensembl_regulatory_build esm1b gerp gnomad4 go metarnn ncbigene omim revel spliceai vest ucscgenomebrowser dbsnp mutationtaster oncokb civic civic_gene -t excel -d ./ --mp 8
